...
# -- Criar o sidebar
with st.sidebar:
    logo_teste = Image.open('./Mídia/logo.png')
    ...
    fProduto = st.selectbox(
        "Selecione o Produto:",
        options=list(df['Produto vendido'].unique())
    )
    fCliente = st.selectbox(
        "Selecione o Cliente:",
        options = list(df['Cliente'].unique())    )
...
tab1_qtde_produto = tab1_qtde_produto.groupby('Produto vendido').count().reset_index()
...
tab3_Vendas_Vendedor = tab3_Vendas_Vendedor.groupby('Vendedor').count().reset_index()
...
tab4_Venda_Cliente = tab4_Venda_Cliente.groupby('Cliente').count().reset_index()
...
dst1, dst2, dst3, dst4 = st.columns([1,1,1,3.5])
with dst1:
